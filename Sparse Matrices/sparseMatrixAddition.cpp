...
//Method to add elements in a similar rows of two sparse matrix
//And to also link with column headers
void addrow(Node** sumr,vector<Node*> &sumc,Node* a,Node* b)
{
	Node* p=a->nextr;
	Node* q=b->nextr;
	Node* r=*sumr;
	while(p!=NULL && q!=NULL)
	{
		...
		{
			Node* temp = createNode(p->row_no,q->column_no,q->data);
			r->nextr=temp;
			r=r->nextr;
	...
	if(p==NULL&&q!=NULL)
	{
		while(q!=NULL)
		{
			Node* temp = createNode(q->row_no,q->column_no,q->data);
			r->nextr=temp;
			Colinsert(&sumc[q->column_no-1],temp);
			q=q->nextr;
			r=temp;
...
int main()
{
	...
	//Add with respect to Rows
	for(int i=0;i<r;i++)
	{
		addrow(&Srow_head[i],Scol_head,Arow_head[i],Brow_head[i]);
	}
	/*for(int i=0;i<r;i++)
	{
		printr(Brow_head[i]);
	...
	//Printing the added Sparse Matrices
	for(int i=0;i<r;i++)
	{
		printr(Srow_head[i]);
	}
